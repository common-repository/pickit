...
trait WooCommerceTrait {
    ...
    public static function get_customer_from_cart($customer) {
        ...
        $pid = self::get_customer_pid($customer);
        ...
        return ['first_name' => $first_name, 'last_name' => $last_name, 'full_name' => $name, 'street' => $address['street'], 'number' => $address['number'], 'floor' => $address['floor'], 'apartment' => $address['apartment'], 'full_address' => $full_address, 'cp' => $postal_code, 'locality' => $locality, 'province' => $province, 'lat' => $latlng['lat'], 'lng'=>$latlng['lng'], 'address_1' =>$address['address1'], 'address_2'=>$address['address2'], 'pid' => $pid];
    }
    ...
    /**
     * Gets the Person Id Customer
     * 
     * @param WC_Customer $customer
     * @return string
     */
     public static function  get_customer_pid($customer) {
        $pid = '1';
        switch (Helper::get_option('fiscal-id-mode')) {
            case 'USE':
                $pid = get_post_meta($customer->get_id() , Helper::get_option('fiscal-id-field'), true);
                break;
            case 'ADD':
                $pid = get_post_meta($customer->get_id() , '_shipping_pickit_fiscal_id', true);
                if(empty($pid)){
                    $pid = get_post_meta($customer->get_id() , '_billing_pickit_fiscal_id', true);                  
                }
                break;
            default:
                $pid = '1';
    ...
    public static function get_latlong($order){
      ...
      if ($order->get_shipping_address_1()) {
          $lat = get_post_meta($order->get_id() , '_shipping_wc_lat', true);
          $lng = get_post_meta($order->get_id() , '_shipping_wc_lng', true);
      }
      else {
          $lat = get_post_meta($order->get_id() , '_billing_wc_lat', true);
          $lng = get_post_meta($order->get_id() , '_billing_wc_lng', true);
      }
    ...
    public static function get_items_from_cart($cart) {
        ...
        foreach ($items as $item) {
            ...
            if(!($product->is_downloadable('yes') || $product->is_virtual('yes'))){
                ...
                if (!$new_product) {
                    self::log_error(__('Helper -> Error obteniendo productos del carrito, producto con malas dimensiones - ID: ', 'wc-pickit') . $product_id);
                    return false;
                }
    ...
    public static function get_items_from_order($order) {
        ...
        foreach ($items as $item) {
            ...
            if(!($product->is_downloadable('yes') || $product->is_virtual('yes'))){
                $new_product = self::get_product_dimensions($product_id);
                if (!$new_product) {
                    self::log_error(__('Helper -> Error obteniendo productos de la orden, producto con malas dimensiones - ID: ', 'wc-pickit') . $product_id);
                    return false;
                }
